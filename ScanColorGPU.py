...
config.enable_stream(other_stream, 1920, 1080, other_format, 30)
...
def run(dt):
    global w, h
    window.set_caption("RealSense (%dx%d) %dFPS (%.2fms) %s%s" %
                       (w, h, 0 if dt == 0 else 1.0 / dt, dt * 1000,
                        "PAUSED" if state.paused else "",
                        "RECORD" if state.record else ""))
    ...
    if keys[pyglet.window.key.E]:
        ...
        start = time.time()
        print("Taking picture ... ", end='')
        filename = "Data/pointclouds/out"
        i = 1
        while True:
            try:
                f = open(filename + str(i) + ".ply")
                f.close()
                i += 1
            except IOError:
                break
        points.export_to_ply(filename + str(i) + ".ply", mapped_frame)
        pyglet.image.get_buffer_manager().get_color_buffer().save(filename + str(i) + ".png")
        print("DONE %.2f sec" % (time.time() - start))

    if keys[pyglet.window.key.SPACE]:
        event_loop.enter_blocking()
        if state.record:
            print("Recorded")
            new_config = rs.config()
            new_config.enable_stream(rs.stream.depth, 1024, 768, rs.format.z16, 30)
            new_config.enable_stream(rs.stream.color, 1920, 1080, rs.format.bgr8, 30)
            pipeline.stop()
            pipeline.start(new_config)
            state.record = False
            keys[pyglet.window.key.SPACE] = False
        else:
            print("Start recording ...")
            filename = "Data/Bags/" + input("Give a name: ")
            new_config = rs.config()
            new_config.enable_stream(rs.stream.depth, 1024, 768, rs.format.z16, 30)
            new_config.enable_stream(rs.stream.color, 1920, 1080, rs.format.bgr8, 30)
            new_config.enable_record_to_file(filename + ".bag")
            pipeline.stop()
            pipeline.start(new_config)
            state.record = True
            keys[pyglet.window.key.SPACE] = False
        event_loop.exit_blocking()
